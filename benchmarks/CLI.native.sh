#!/usr/bin/env bash
# Native-render benchmark runner: cellestial -> HTML, scanpy -> PNG.
#
#   bash benchmarks/CLI.native.sh                    # run everything
#   source benchmarks/CLI.native.sh && run_heatmap   # run one case
#
# Each script appends to benchmarks/results.native.feather (created if absent) and
...
SCRIPTS_DIR="$(cd "$(dirname "$_self")" && pwd)/scripts/natives"
