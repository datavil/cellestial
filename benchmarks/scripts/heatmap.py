...
heatmap, then appends the results to ``benchmarks/results.feather``. The number
of genes is controlled by ``-n`` and is recorded as ``n_cols``.

Run from the repo root::

    poetry run python benchmarks/scripts/heatmap.py -d data/pbmc3k_pped.h5ad -n 20 -r 1
"""
