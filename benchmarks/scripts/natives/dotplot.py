"""
Dotplot benchmark: cellestial vs scanpy.

Measures peak memory and time for building and rendering a gene-by-group
dotplot, then appends the results to ``benchmarks/results.native.feather``. The number
of genes is controlled by ``-n`` and is recorded as ``n_cols``.

Run from the repo root::

    poetry run python benchmarks/scripts/natives/dotplot.py -d data/pbmc3k_pped.h5ad -n 20 -r 1
"""
...
def _cl_dotplot(data, genes: list[str], group_by: str) -> None:
    plot = cl.dotplot(data, keys=genes, group_by=group_by, tooltips="none")
    _render_cellestial(plot)


def _sc_dotplot(data, genes: list[str], group_by: str) -> None:
    sc.pl.dotplot(data, var_names=genes, groupby=group_by, show=False)
    _render_scanpy(None)
...
def main() -> None:
    ...
    cellestial_memory, cellestial_time = _measure(lambda: _cl_dotplot(data, genes, group_by))
    scanpy_memory, scanpy_time = _measure(lambda: _sc_dotplot(data, genes, group_by))
