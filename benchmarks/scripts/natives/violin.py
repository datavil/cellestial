"""
Violin (boxplot slot) benchmark: cellestial vs scanpy.

scanpy.pl has no boxplot, so both libraries render a violin of a single gene's
expression split by a categorical group. Measures peak memory and time, then
appends the results to ``benchmarks/results.native.feather`` under case ``violin``.

Run from the repo root::

    poetry run python benchmarks/scripts/natives/boxplot.py -d data/pbmc3k_pped.h5ad -r 1
"""
...
def _cl_violin(data, gene: str, group_by: str) -> None:
    plot = cl.violin(data, key=gene, group_by=group_by, tooltips="none", show_points=False)
    _render_cellestial(plot)


def _sc_violin(data, gene: str, group_by: str) -> None:
    sc.pl.violin(data, keys=gene, groupby=group_by, show=False, stripplot=False)
    _render_scanpy(None)
...
def main() -> None:
    ...
    cellestial_memory, cellestial_time = _measure(lambda: _cl_violin(data, gene, group_by))
    scanpy_memory, scanpy_time = _measure(lambda: _sc_violin(data, gene, group_by))
